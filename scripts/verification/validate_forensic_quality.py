...
files = ['june_pixel_dense_v2.csv', 'feb_pixel_dense_v2.csv']
...
for f_name in files:
    ...
    try:
        ...
        print(f"Unique Timestamps: {df['timestamp'].nunique()} (Duplicates: {df['timestamp'].duplicated().sum()})")
        print(f"Max Rate of Change: {roc.max():.4f} mmol/L/min (Physiological max: 0.3-0.4)")
        print(f"Glucose Range: {df['glucose'].min():.2f} to {df['glucose'].max():.2f} mmol/L")
        
        # Check for gaps
        gaps = dt_mins[dt_mins > 30]
        print(f"Number of sensor gaps (>30m): {len(gaps)}")
