...
def plot_glucose_data(csv_path, output_image=None, days_to_show=None, smooth_window=None):
    ...
    for ax, data, title in plot_segments:
        ...
        # 1. Main Trace: Pixel-Perfect Reconstruction
        # We use a solid line + high-density scatter to 'feel' the thousands of points
        ax.plot(data_clean['timestamp'], data_clean['glucose'], color='#0984e3', 
                linewidth=2.8, alpha=0.9, zorder=5, label='Glucose (Pixel-Dense Trace)')
        
        # Subtle 'Pixel' Scatter to show the raw data density
        ax.scatter(data_clean['timestamp'], data_clean['glucose'], color='#0984e3', 
                  s=12, alpha=0.15, edgecolors='none', zorder=6)
        
        # High-Fidelity Curvy Overlay (PCHIP) if enough data exists
        if len(data_clean) > 5:
            x = mdates.date2num(data_clean['timestamp'])
            y = data_clean['glucose'].values
        ...
        # Ultra-Density Time Axes
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%a %d, %H:%M'))
        ax.xaxis.set_major_locator(mdates.HourLocator(byhour=[0, 6, 12, 18])) # 6-hour majors
        ax.xaxis.set_minor_locator(mdates.HourLocator(interval=1)) # 1-hour minors
        
        ax.grid(True, which='major', linestyle='-', alpha=0.25)
        ax.grid(True, which='minor', linestyle=':', alpha=0.15)
        ax.set_facecolor('#fdfdfd')
        ax.set_ylim(0, max(data_clean['glucose'].max() + 3, 16))
