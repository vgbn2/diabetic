...
class RealTimeHUD:
    ...
    def _get_header(self) -> Panel:
        ...
        grid.add_row(
            Text("BIO-QUANT PREDICTOR v2.0", style="bold cyan"),
    ...
    def _get_metrics_table(self, snapshot: Optional[MetabolicSnapshot], prediction: float) -> Table:
        ...
        if snapshot:
            table.add_row("Glucose", f"{snapshot.filtered_value:.1f}", "mmol/L" if config.PREFER_MMOL else "mg/dL")
            table.add_row("Velocity", f"{snapshot.velocity:+.2f}", "mg/dL/min")
            table.add_row("Acceleration", f"{snapshot.acceleration:+.3f}", "mg/dL/min²")
